...
function mango(target, transmitCommand, uiIndex) {
	...
	var gravity = 0.1;
	var bounceLoss = -1; // 0.7
	var framerate = 60;
	var slingAffect = 10; //10 -- lower number means more speed
	var ballsize = 5; // 7
	var slingSize = 40;
	...
	this.addBlocks = function() {
		...
		if (Math.abs(self.CurrentBlocks[self.CurrentBlocks.length-1].x - self.CurrentBalls[0].x)<50) {
			console.log(Math.abs(self.CurrentBlocks[self.CurrentBlocks.length-1].x - self.CurrentBalls[0].x));
			var newBlock = { x: self.CurrentBalls[0].x+200+(~~(Math.random()*50)),  y: ~~(Math.random()*self.height), width: 50, height: 5, type: "land" };
			self.CurrentBlocks.push(new Block(self.CurrentBlocks.length, newBlock));
		}
